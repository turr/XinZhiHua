jQuery(function () {
    ...
    var InitUpload = function (box) {
       ...
       uploader;
        ...
        uploader = WebUploader.create({
            ...
            // 文件接收服务端。
            server: '/manager/UploadImg?key=' + type+'&from=product',
    ...
    var UpdateData = function (box) {
        ...
        var type = box.find("input[name='type']").val();
        var product_no = box.find("input[name='product_no']").val();
        var product_type = box.find("input[name='product_type']").val();


        $.ajax({
            cache: false,
            url: "/manager/ProductEditPost",
            type: "post",
            async: false,
            data: {
                id: id,
